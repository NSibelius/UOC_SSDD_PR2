...
//		LSimLogger.log(Level.TRACE,"The recipe '"+recipeTitle+"' has been added");
	...
	//MIO
	public synchronized void DeliverMessages(TimestampVector oldSummary)
	{
		...
		{
			messages = getLog().msgList(pid, oldSummary.getLast(pid));
			...
			for(Operation op : messages)
			{
				if(op.getType()==OperationType.ADD)
				{
					...
					{
						Recipe rcpe = new Recipe(a.getRecipe().getTitle(), a.getRecipe().getRecipe(), pid,a.getTimestamp());
						recipes.add(rcpe);
					}
				}
				if(op.getType()==OperationType.REMOVE)
				{
					RemoveOperation r = (RemoveOperation) op;
