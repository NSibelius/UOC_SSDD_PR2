...
public class TSAESessionPartnerSide extends Thread{
	...
	private TimestampVector originatorSummary;
	
	//A temporary copy of the local summary and ack vectors, to avoid timing problems with concurrent sessions
	private TimestampVector localSummary;
	private TimestampMatrix localAck;
	...
	public void run() {
		...
		try {
			...
			if (msg.type() == MsgType.AE_REQUEST){
				...
				originatorSummary = request.getSummary(); 
				messages= serverData.getLog().listNewer(originatorSummary); 
				...
				while (msg.type() == MsgType.OPERATION){
					...
					if(serverData.getLog().add(response.getOperation()))
					{
						AddOperation a = (AddOperation) response.getOperation();
						Recipe rcpe = new Recipe(a.getRecipe().getTitle(), a.getRecipe().getRecipe(), a.getTimestamp().getHostid(),a.getTimestamp());
